...
class QueueTestPushPeek : public stest::LeafTest {
public:
    QueueTestPushPeek() : LeafTest("push-peek") {
    }
 ...
 */
template<class QueueT>
class QueueTestPop : public stest::LeafTest {
public:
    QueueTestPop() : LeafTest("pop") {
    }
 ...
 */
template<class QueueT>
class QueueTestClear : public stest::LeafTest {
public:
    QueueTestClear() : LeafTest("clear") {
    }
 ...
 */
template<class QueueT>
class QueueTestCopyAssignEquals : public stest::LeafTest {
public:
    QueueTestCopyAssignEquals() : LeafTest("copy-assign-equals") {
    }
...
protected:
    void test() override {
        ...
        if constexpr (std::is_same_v<QueueT, adt::ImplicitQueue<int>>) {
            ...
            queue5.push(2);
            this->assert_false(
                queue4.equals(queue5),
                "Full different implicit queues are different"
            );
        }
    }
};
 ...
 */
template<class QueueT>
class GeneralQueueTest : public stest::CompositeTest {
public:
    GeneralQueueTest(const std::string &name) : CompositeTest(name) {
 ...
 */
class QueueTest : public stest::CompositeTest {
public:
    QueueTest() : CompositeTest("Queue") {
        this->add_test(
            std::make_unique<GeneralQueueTest<adt::ImplicitQueue<int>>>(
                "ImplicitQueue"
            )
        );
        this->add_test(
            std::make_unique<GeneralQueueTest<adt::ExplicitQueue<int>>>(
                "ExplicitQueue"
            )
        );
    }
