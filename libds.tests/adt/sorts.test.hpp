...
constexpr auto makeReverseComparator = [](const auto &cmp) {
    return [cmp](const auto &l, const auto &r) { return cmp(r, l); };
};
...
public:
    ...
    amt::ImplicitSequence<int> operator() (const std::size_t n) {
        auto keys = amt::ImplicitSequence<int>(n, false);
        for (std::size_t i = 0; i < n; ++i) {
            keys.emplaceLast().data = distKey_(rngKey_);
        }
        return keys;
    }
...
using ReverseStringLengthComparator
    = decltype(makeReverseComparator(StringLengthComparator()));
...
using ReverseStringPtrComparator
    = decltype(makeReverseComparator(StringPtrComparator()));
} // namespace details
 ...
 */
template<class SortT, class Comparator, class KeyGenerator, class KeyDisposer>
class GenericSortTest : public stest::LeafTest {
public:
    GenericSortTest(
...
protected:
    void test() override {
        ...
        this->assert_true(
            std::is_sorted(keys.begin(), keys.end(), cmp_),
            "Is sorted."
        );
        keyDisposer_(keys);
    }
...
class MultiCmpSortTest : public stest::CompositeTest {
public:
    MultiCmpSortTest(
        const std::string &name,
        std::mt19937_64 &seeder,
        std::initializer_list<int> elementCounts
    ) :
        CompositeTest(name) {
        using namespace details;
        for (const auto n : elementCounts) {
            this->add_test(
                std::make_unique<GenericSortTest<
                    SortT<int>,
                    std::less<>,
                    RandomIntKeyGenerator,
                    IntKeyDisposer>>(
 ...
 */
class RadixSortTest :
    public GenericSortTest<
        adt::RadixSort<int, int>,
        std::less<>,
        details::RandomIntKeyGenerator,
 ...
 */
class SortTest : public stest::CompositeTest {
public:
    SortTest() : CompositeTest("Sort") {
        auto seeder = std::mt19937_64(247);

        const auto smallNs = {1, 2, 3, 100};
        const auto bigNs = {1, 2, 3, 10'000};

        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::SelectSort>>(
                "SelectSort",
                seeder,
                smallNs
            )
        ...
        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::BubbleSort>>(
                "BubbleSort",
                seeder,
                smallNs
            )
        ...
        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::InsertSort>>(
                "InsertSort",
                seeder,
                smallNs
            )
        ...
        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::QuickSort>>(
                "QuickSort",
                seeder,
                bigNs
            )
        ...
        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::HeapSort>>(
                "HeapSort",
                seeder,
                bigNs
            )
        ...
        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::ShellSort>>(
                "ShellSort",
                seeder,
                bigNs
            )
        ...
        this->add_test(
            std::make_unique<MultiCmpSortTest<adt::MergeSort>>(
                "MergeSort",
                seeder,
                bigNs
            )
