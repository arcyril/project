...
class StackTestPushPeek : public stest::LeafTest {
public:
    StackTestPushPeek() : LeafTest("push-peek") {
    }
 ...
 */
template<class StackT>
class StackTestPop : public stest::LeafTest {
public:
    StackTestPop() : LeafTest("pop") {
    }
 ...
 */
template<class StackT>
class StackTestClear : public stest::LeafTest {
public:
    StackTestClear() : LeafTest("clear") {
    }
 ...
 */
template<class StackT>
class StackTestCopyAssignEquals : public stest::LeafTest {
public:
    StackTestCopyAssignEquals() : LeafTest("copy-assign-equals") {
    }
...
protected:
    void test() override {
        ...
        this->assert_true(
            ...
            "Copy constructed stack is the same."
        );
        stack1.pop();
        this->assert_false(
            stack1.equals(stack2),
        ...
        stack1.pop();
        this->assert_false(
            stack1.equals(stack3),
            "Modified assigned stack is different."
        );
    }
};
 ...
 */
template<class StackT>
class GeneralStackTest : public stest::CompositeTest {
public:
    GeneralStackTest(const std::string &name) : CompositeTest(name) {
 ...
 */
class StackTest : public stest::CompositeTest {
public:
    StackTest() : CompositeTest("Stack") {
        this->add_test(
            std::make_unique<GeneralStackTest<adt::ImplicitStack<int>>>(
                "ImplicitStack"
            )
        );
        this->add_test(
            std::make_unique<GeneralStackTest<adt::ExplicitStack<int>>>(
                "ExplicitStack"
            )
        );
    }
