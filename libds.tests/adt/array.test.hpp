 ...
 */
class ArrayTestGetters : public stest::LeafTest {
public:
    ArrayTestGetters() : LeafTest("getters") {
    }
 ...
 */
class ArrayTestSetAccess : public stest::LeafTest {
public:
    ArrayTestSetAccess() : LeafTest("set-access") {
    }
 ...
 */
class ArrayTestCopyAssignEquals : public stest::LeafTest {
public:
    ArrayTestCopyAssignEquals() : LeafTest("copy-assign-equals") {
    }
...
protected:
    void test() override {
        ...
        this->assert_true(
            arr1.equals(arr2),
            "Copy constructed array is the same."
        );
        arr1.set(-1, base);
        ...
        this->assert_false(
            arr1.equals(arr3),
            "Modified assigned array is different."
        );

        auto arr4 = adt::Array<int>(adt::Dimension(base, size + 10));
        this->assert_throws([&arr4, &arr3]() { arr4.assign(arr3); });
    }
};
 ...
 * @brief Tests simple queries.
 */
class MatrixTestGetters : public stest::LeafTest {
public:
    MatrixTestGetters() : LeafTest("getters") {
    }
 ...
 * @brief Tests set and access.
 */
class MatrixTestSetAccess : public stest::LeafTest {
public:
    MatrixTestSetAccess() : LeafTest("set-access") {
    }
...
protected:
    void test() override {
        ...
        for (long long i = d1.getBase();
             i < d1.getBase() + static_cast<long long>(d1.getSize());
             ++i) {
            for (long long j = d2.getBase();
                 j < d2.getBase() + static_cast<long long>(d2.getSize());
                 ++j) {
 ...
 * @brief Tests copy constructor, assign, and equals.
 */
class MatrixTestCopyAssignEquals : public stest::LeafTest {
public:
    MatrixTestCopyAssignEquals() : LeafTest("copy-assign-equals") {
    }
...
protected:
    void test() override {
        ...
        this->assert_true(
            ...
            "Copy constructed matrix is the same."
        );
        ...
        this->assert_true(
            ...
            "Assigned matrix is the same."
        );
        matrix3.set(-1, d1.getBase() + 1, d2.getBase() + 1);
        matrix3.set(-1, d1.getBase() + 2, d2.getBase() + 2);
        this->assert_false(
            matrix1.equals(matrix3),
            "Modified assigned matrix is different."
        );

        this->assert_throws([&]() {
            auto matrix4 = adt::CompactMatrix<int>(
                adt::Dimension(d1.getBase() + 1, d1.getSize() - 1),
                d2
            );
        ...
        this->assert_throws([&]() {
            auto matrix4 = adt::CompactMatrix<int>(
                d1,
                adt::Dimension(d2.getBase() + 1, d2.getSize() - 1)
            );
            matrix4.assign(matrix3);
        });
    }
 ...
 */
class ArraysTest : public stest::CompositeTest {
public:
    ArraysTest() : CompositeTest("Arrays") {
