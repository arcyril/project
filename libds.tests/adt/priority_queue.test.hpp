#ifndef LIBDS_TESTS_ADT_PRIORITY_QUEUE_TEST_HPP
#define LIBDS_TESTS_ADT_PRIORITY_QUEUE_TEST_HPP
...
class PrioQueueTestBase : public stest::LeafTest {
public:
...
public:
    PrioQueueTestPush() : details::PrioQueueTestBase<PrioQueueT>("push") {
    }
...
public:
    PrioQueueTestPop() : details::PrioQueueTestBase<PrioQueueT>("pop") {
    }
...
protected:
    void test() override {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            this->assert_true(
                popped >= prevPrio,
                "Popped prio is smaller than previously popped"
            );
            prevPrio = popped;
        }
...
public:
    PrioQueueTestClear() : details::PrioQueueTestBase<PrioQueueT>("clear") {
    }
...
protected:
    void test() override {
        constexpr int n = 30;
        PrioQueueT queue1
            = details::PrioQueueTestBase<PrioQueueT>::makeQueue(n);
        for (int i = 0; i < n; ++i) {
            const int newPriority = this->generateRandomPriority();
        ...
        this->assert_true(
            this->bruteforceEquals(queue1, queue2),
            "Copy constructed queue is the same."
        );
        queue1.pop();
        this->assert_false(
            this->bruteforceEquals(queue1, queue2),
        ...
        this->assert_true(
            this->bruteforceEquals(queue1, queue3),
            "Assigned queue is the same."
        );
        queue1.pop();
        queue1.pop();
        this->assert_false(
            this->bruteforceEquals(queue1, queue3),
            "Modified assigned queue is different."
        );
        ...
        queue5.pop();
        this->assert_true(
            this->bruteforceEquals(queue4, queue5),
            "Modified copies are the same"
        );
    }
};
 ...
 */
template<class PrioQueueT>
class GeneralPrioQueueTest : public stest::CompositeTest {
public:
    GeneralPrioQueueTest(const std::string &name) : CompositeTest(name) {
 ...
 */
class TwoListsScenario1 :
    public details::PrioQueueTestBase<adt::TwoLists<int, int>> {
public:
    TwoListsScenario1() :
        details::PrioQueueTestBase<adt::TwoLists<int, int>>(
            "TwoListsScenario1"
        ) {
    }
...
protected:
    void test() override {
        ...
        while (! queue.isEmpty()) {
            ...
            this->assert_true(
                prev <= current,
                "Popped priority is smaller than previously popped"
            );
            prev = current;
        }
    }
 ...
 */
class TwoListsTest : public stest::CompositeTest {
public:
    TwoListsTest() : CompositeTest("TwoLists") {
        this->add_test(
            std::make_unique<GeneralPrioQueueTest<adt::TwoLists<int, int>>>(
                "TwoLists-GenericTest"
            )
        );
 ...
 */
class PriorityQueueTest : public stest::CompositeTest {
public:
    PriorityQueueTest() : CompositeTest("PriorityQueue") {
        this->add_test(
            std::make_unique<GeneralPrioQueueTest<
                adt::UnsortedImplicitSequencePriorityQueue<int, int>>>(
                "UnsortedImplicit"
            )
        );
        this->add_test(
            std::make_unique<GeneralPrioQueueTest<
                adt::UnsortedExplicitSequencePriorityQueue<int, int>>>(
                "UnsortedExplicit"
            )
        );
        this->add_test(
            std::make_unique<GeneralPrioQueueTest<
                adt::SortedImplicitSequencePriorityQueue<int, int>>>(
                "SortedImplicit"
            )
        );
        this->add_test(
            std::make_unique<GeneralPrioQueueTest<
                adt::SortedExplicitSequencePriorityQueue<int, int>>>(
                "SortedExplicit"
            )
        );
        this->add_test(
            std::make_unique<GeneralPrioQueueTest<adt::BinaryHeap<int, int>>>(
                "BinaryHeap"
            )
        );
